...
def handleSession(user_session, packet_inicial):
    ...
    while estado == Estado.SYNCING and loops < MAX_LOOPS:
        ...
        if packet is not None and packet.connect == 1:
            #si no es un paquete connect no tiene sentido revisarlo en este punto
            if waiting_sinack:
                ...
                response = Packet(user_id, flags=Packet.FLAG_CONNECT | Packet.FLAG_SYN | Packet.FLAG_ACK)
                user_session.sock.sendto(response.toBytes(), user_session.addr)
                print(f"Enviando SYNACK al usuario {user_id} (iteraciones: {loops})")
    ...
    # Loop para probar que recibe mensajes del cliente y envia un ack generico por cada uno,
    #si despues de 2 segundos no recibe nada cierra la sesion
    while user_session.get_estado() == Estado.ACTIVE:
        try:
            packet = user_session.queue.get(timeout=TIMEOUT)
            print(f"[ACTIVE] Recibido de userId {user_id}: {packet.to_string()}")
            ...
            print(f"[ACTIVE] Timeout esperando mensaje de userId {user_id}, cerrando sesión activa.")
            break
    ...
    try:
        while True:
            ...
            if user_id == USER_ID_NUEVO:
                ...
                t = threading.Thread(target=handleSession, args=(user_session, packet), daemon=True)
                t.start()
            elif user_id in server_sessions:
                user_session = server_sessions[user_id]
                user_session.queue.put(packet)
            else:
                print(f"Paquete descartado: user_id {user_id} no reconocido.")
