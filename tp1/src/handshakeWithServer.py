...
def handshake_with_server(clientsocket, server, servicio, nombre_archivo_server):
    ...
    for intento_envio in range(max_envios):
        try:
            ...
            logger.info(f"[Handshake] Enviado paquete inicial, intento {intento_envio+1}/{max_envios}")
        ...
        for intento_recv in range(max_recvs):
            try:
                ...
                if conexion_ok:
                    ...
                    logger.error("Detalles de conexión inválidos. Abortando handshake actual.")
                    break  # Sale del ciclo interno y reintenta el envío externo
            except Exception as e:
                logger.error(f"Error en handshake {e}")
                logger.debug(f"intento de recibir SIN+ACK: {intento_recv+1}/{max_recvs}")
                if intento_recv == max_recvs - 1:
                    logger.debug("No se recibió respuesta tras 10 intentos de recv. Reintentando envío inicial...")
        #no pudo recibir el user id, se vuelve a intentar el envio inicial
    logger.error("No se pudo completar el handshake tras 3 intentos. Proceso abortado.")
    exit(1)
...
def recibir_detalles_conexion(payload):
    ...
    logger.error("Formato de respuesta del server invalido, esperando nuevo mensaje")
    return False